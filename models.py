...
class M1(object):
    def __init__(self, x_dim, z_dim):
        ...
        with self.G.as_default():
            ...
            self.encoder_hidden_z0_batch_norm = tf.layers.batch_normalization(self.encoder_hidden_z0,
                                                                              training=self.phase)
            ...
            # Batch Normalization before activation
            self.decoder_hidden_z0_batch_norm = tf.layers.batch_normalization(self.decoder_hidden_z0,
                                                                              training=self.phase)
    ...
    def train(self, x, epochs, batch_size, learning_rate=0.0003, beta1=0.9, beta2=0.999, save_path=None, load_path=None,
        ...
        if save_path is None:
            self.save_path = "model_M1/VAE.ckpt"
        else:
...
class M2(object):
    def __init__(self, z1_dim, z2_dim, y_dim, num_examples, num_labelled, num_batches, alpha=0.1):
        ...
        with self.G.as_default():
            ...
            self.classifier_hidden_h0 = tf.nn.softplus(tf.matmul(self.labelled_z1, self.classifier_hidden_w0) + self.classifier_hidden_b0)
            ...
            self.encoder_hidden_h0 = tf.nn.softplus(tf.matmul(self.encoder_combined_input, self.encoder_hidden_w0) + self.encoder_hidden_b0)
            ...
            self.decoder_hidden_h0 = tf.nn.softplus(tf.matmul(self.decoder_combined_input, self.decoder_hidden_w0) + self.decoder_hidden_b0)
            ...
            self.predictor_hidden_h0 = tf.nn.softplus(tf.matmul(self.unlabelled_z1, self.classifier_hidden_w0) + self.classifier_hidden_b0)
    ...
    def train(self, z1, y, unlabelled_z1, epochs, z1_valid, y_valid, learning_rate=0.0003, beta1=0.9, beta2=0.999,
        ...
        if save_path is None:
            self.save_path = "model_M2/GC.cpkt"
        else:
            self.save_path = save_path
